...
void pid(int pid_val[4]) {

  pid_val[0] = map(outputPitch + outputRoll, -100, 100, 4000, 8000);
  pid_val[1] = map(-outputPitch + outputRoll, -100, 100, 4000, 8000);
  pid_val[2] = map(-outputPitch - outputRoll, -100, 100, 4000, 8000);
  pid_val[3] = map(outputPitch - outputRoll, -100, 100, 4000, 8000);
}
...
void CalibrateInitialAngle() {
  ...
  setpointRoll = sumRoll / 10;
  setpointPitch = sumPitch / 10;
...
void loop() {
  ...
  if (s_cDataUpdate & ANGLE_UPDATE) {
    ...
    if (input.length() == 52) {
      ...
      int pid_val[4];
      //pid(pid_val[4]);
      pid_val[0] = map(outputPitch + outputRoll, -100, 100, -2000, 2000);
      pid_val[1] = map(-outputPitch + outputRoll, -100, 100, -2000, 2000);
      pid_val[2] = map(-outputPitch - outputRoll, -100, 100, -2000, 2000);
      pid_val[3] = map(outputPitch - outputRoll, -100, 100, -2000, 2000);

      
      int Thruster1 = constrain(PWM[4] + pid_val[0], 4000, 8000);
      int Thruster4 = constrain(PWM[4] + pid_val[1], 4000, 8000);
      int Thruster3 = constrain(PWM[4] + pid_val[2], 4000, 8000);
      int Thruster2 = constrain(PWM[4] + pid_val[3], 4000, 8000);
